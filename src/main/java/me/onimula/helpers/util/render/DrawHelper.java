package me.onimula.helpers.util.render;
...
import me.onimula.Wrapper;
import me.onimula.helpers.util.misc.ColorHelper;
import me.onimula.helpers.util.misc.Utils;
...
public class DrawHelper implements Wrapper {
	...
	public static void drawRoundedOutline(float x, float y, float width, float height, float radius, float thickness, Color color) {
		...
		for (int i = 0; i <= 360; i += 10) {
			...
			// Внешний край
			bufferBuilder.vertex(x + radius + (radius - halfThickness) * cos, y + radius + (radius - halfThickness) * sin, 0).endVertex();
			// Внутренний край
			bufferBuilder.vertex(x + radius + (radius + halfThickness) * cos, y + radius + (radius + halfThickness) * sin, 0).endVertex();
		}
